      ...
      <div class="form-group mt-5">
        ...
        <div class="custom-file mb-3 w-20">
          <input type="file" class="custom-file-input" name="category_image" 
            required @change="processImage($event)"
...
export default {
  ...
  methods: {
    processImage(event) {
      ...
      document.getElementById('selected-category-image').src = window.URL.createObjectURL(this.image);
    },
    ...
    submit() {
      ...
      this.busy = true;
      this.$axios.post(`${this.$hostname}/api/admin/ads`, formdata)
        .then(res => {
          this.$router.push("/ads");
        })
        .catch(err => {
