...
export default {
  ...
  created() {
    this.$axios.get(`${this.$hostname}/api/admin/ads/${this.id}`)
      .then(res => {
  ...
  methods: {
    processImage(event) {
      ...
      document.getElementById('selected-category-image').src = window.URL.createObjectURL(this.newimage);
    },
    ...
    submit() {
      ...
      this.busy = true;
      this.$axios.post(`${this.$hostname}/api/admin/ads/${this.id}`, formdata)
        .then(res => {
          this.$router.push("/ads");
        })
        .catch(err => {
